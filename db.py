...
class Database:
    ...
    def add_stadium(self, owner_id, name, link, lat, lon, price, hours, photo):
        with self.connection:
            return self.cursor.execute("INSERT INTO stadiums (owner_id, name, address_link, latitude, longitude, price_per_hour, work_hours, photo_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", (owner_id, name, link, lat, lon, price, hours, photo))
    ...
    def add_booking(self, user_id, s_id, date, slot):
        with self.connection:
            return self.cursor.execute("INSERT INTO bookings (user_id, stadium_id, booking_date, time_slot) VALUES (?, ?, ?, ?)", (user_id, s_id, date, slot))

    def get_booked_slots(self, s_id, date):
        with self.connection:
            res = self.cursor.execute("SELECT time_slot FROM bookings WHERE stadium_id = ? AND booking_date = ? AND status = 'active'", (s_id, date)).fetchall()
