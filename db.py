...
class Database:
    def __init__(self, db_file):
        # SQLite sync, lekin multi-thread safe
    ...
    def get_stadium_by_name(self, name):
        with self.connection:
            return self.cursor.execute(
                "SELECT * FROM stadiums WHERE name LIKE ?",
                (f"%{name}%",)
            ).fetchone()
    ...
    def get_owner_stadiums(self, owner_id):
        with self.connection:
            return self.cursor.execute(
                "SELECT * FROM stadiums WHERE owner_id=?",
                (owner_id,)
            ).fetchall()
    ...
    def get_all_teams(self):
        with self.connection:
            return self.cursor.execute(
                "SELECT t.*, u.full_name FROM teams t JOIN users u ON t.creator_id=u.id WHERE t.joined_players < t.needed_players ORDER BY t.id DESC"
            ).fetchall()
    ...
    def join_team(self, team_id):
        with self.connection:
            self.cursor.execute(
                "UPDATE teams SET joined_players = joined_players + 1 WHERE id=? AND joined_players < needed_players",
                (team_id,)
            )
